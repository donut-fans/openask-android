...
val Throwable.errorMsg: String
    get() {
        return if (this is UnknownHostException) { //网络异常
            if (!isNetworkConnected(OpenAskApplication.instance.applicationContext))
                OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_nonetwork)
            else
                OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_net_unavailable)
        } else if (
            ...
            OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_timeout)
        } else if (this is ConnectException) {
            OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_net_slow)
        } else if (this is HttpStatusCodeException) {               //请求失败异常
            OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_state_error)+errorCode
        } else if (this is JsonSyntaxException) {  //请求成功，但Json语法异常,导致解析失败
            OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_parse_error)
        } else if (this is ParseException) {       // ParseException异常表明请求成功，但是数据不正确
            message+""   //msg为空，显示code
        } else {
            OpenAskApplication.instance.applicationContext.getString(R.string.errmsg_state_error)
        }
    }
